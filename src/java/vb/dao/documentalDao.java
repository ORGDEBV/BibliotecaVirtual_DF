 ...
 */
public class documentalDao implements entidadService<Biblioteca> {

    sql conector = new sql();

    @Override
    public int crearEntidad(Biblioteca t) {
    ...
    @Override
    public int actualizarEntidad(Biblioteca t) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
    ...
    public int validarTitulo(Documental documental) {
        ...
        data = conector.execProcedure("BV.SP_VALIDA_TITULO", parametros);
        for (Object[] dat : data) {
    ...
    public String generarIdDocumental(int Tipo) {
        ...
        data = conector.execProcedure("[BV].[SP_GENERAR_ID_DOCUMENTAL]", parametros);
        for (Object[] dat : data) {
            n = dat[0].toString();
        }
        return n;
